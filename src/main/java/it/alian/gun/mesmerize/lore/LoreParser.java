...
public class LoreParser {
    ...
    private static final Map<Long, LoreInfo> infoMap = Collections.synchronizedMap(new HashMap<>());

    public static void remove(long id) {
        infoMap.remove(id);
    }

    public static LoreInfo getByEntityId(long id) {
        return infoMap.getOrDefault(id, LoreInfo.empty());
    }

    public static ItemInfo parseItem(Item itemStack) {
        if (itemStack != null && itemStack.getLore() != null)
     ...
     */
    public static boolean check(Item itemStack, Entity user) {
        ItemInfo itemInfo = parseItem(itemStack);
        boolean cancel = false;
        if (itemInfo.soulbound != null && !itemInfo.soulbound.equals(user.getName())) {
            if (user instanceof Player)
                ((Player) user).sendMessage(MConfig.Message.omSoulboundCheck);
            cancel = true;
        }
        if (user instanceof Player && itemInfo.levelCap > ((Player) user).getExperienceLevel()) {
            MTasks.execute(() -> ((Player) user).sendMessage(String.format(MConfig.Message.onLevelCheck, itemInfo.levelCap)));
            cancel = true;
        }
        if (itemInfo.permission != null && user instanceof Player) {
            if (!((Player) user).hasPermission(MConfig.General.permissionAlias.getOrDefault(itemInfo.permission, itemInfo.permission))) {
                MTasks.execute(() -> ((Player) user).sendMessage(String.format(MConfig.Message.onPermissionCheck, itemInfo.permission)));
                cancel = true;
            }
        }
        return cancel;
    ...
    public static LoreInfo parseSingleItem(Item itemStack) {
    ...
    public static class ParseEntityTask implements Callable<LoreInfo> {
        ...
        public ParseEntityTask(EntityLiving entity) {
            this.entity = entity;
        }
        ...
        public LoreInfo call() {
            if (entity.isValid() && entity.isAlive()) {
                ...
                for (Item itemStack : Equipment.collect(entity)) {
        ...
        private static void parseItem(LoreInfo loreInfo, Item itemStack) {
            if (itemStack != null && itemStack.getLore() != null) {
    ...
    public static void init() {
        Server.getInstance().getScheduler().scheduleDelayedRepeatingTask(Mesmerize.instance, () -> {
            Map<Long, Future<LoreInfo>> map = new HashMap<>();
            for (Level world : Server.getInstance().getLevels().values()) {
                for (Entity livingEntity : world.getEntities()) {
                    ...
                    map.put(livingEntity.getId(), MTasks.submit(new ParseEntityTask((((EntityLiving) livingEntity)))));
                }
            }
            MTasks.execute(() -> {
                synchronized (infoMap) {
                    infoMap.clear();
                    for (Map.Entry<Long, Future<LoreInfo>> entry : map.entrySet()) {
                        try {
        ...
        }, 0, ((int) MConfig.Performance.loreUpdateInterval));
    }
